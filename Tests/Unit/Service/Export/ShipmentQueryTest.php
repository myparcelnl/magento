...
it('splits a long id list over several calls, so the url stays servable', function () {
    // Every id goes into one path segment. 250 of them in one go is a ~2 KB segment today and a
    // four-figure selection passes nginx's 8 KB header limit, which fails before the API sees it.
    $calls = [];

    shipmentQueryFor(shipmentsResponse([]), $calls)->findMany(range(1, 250));

    expect($calls)->toHaveCount(3)
        ->and(substr_count($calls[0][0], ';'))->toBe(99)
        ->and(substr_count($calls[2][0], ';'))->toBe(49)
        ->and($calls[2][0])->toEndWith('250');
});

it('merges the shipments of every chunk into one map', function () {
    $calls = [];
    $api   = Mockery::mock(ShipmentApi::class);
    $api->shouldReceive('getShipmentsById')->andReturnUsing(
        function (string $ids) use (&$calls) {
            $calls[] = $ids;

            // One shipment per id asked for, so a dropped chunk shows up as a missing key.
            return shipmentsResponse(array_map(
                static fn(string $id) => (new ShipmentDefsShipment())->setId((int) $id),
                explode(';', $ids)
            ));
        }
    );

    $shipments = (new ShipmentQuery($api))->setUserAgents(createUserAgent()->map())->findMany(range(1, 150));

    expect($calls)->toHaveCount(2)
        ->and($shipments)->toHaveCount(150)
        ->and($shipments)->toHaveKey(1)
        ->and($shipments)->toHaveKey(150);
});