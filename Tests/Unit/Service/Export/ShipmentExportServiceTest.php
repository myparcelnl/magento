...
/**
 * the export scenarios. Fixtures live in Tests/Helpers/ExportFixtures.php.
 *
...
it('reports an unattributable chunk rejection once, not once per order', function () {
    ...
    //
    // The spy fails with a plain RuntimeException, which is not a 422 — so the wording is the
    // cautious one. Nothing here knows whether the call was processed.
    expect($report->hasFailures())->toBeTrue()
        ...
        ->and($report->failureMessages())->toBe([
            'MyParcel could not be reached for 2 orders, so it is unknown whether they shipped.'
            . ' Check them in the MyParcel backoffice before running the action again:'
            . ' the API refused this chunk',
        ]);
});
...
it('reports a shipment whose id could not be stored as created, not as refused', function () {
    ...
    expect($report->succeeded())->toBeEmpty()
        ->and(implode('; ', $report->failureReasons()['100000001']))
...
it('keeps a failure visible when a sibling collo of the same order succeeded', function () {
    ...
    expect($report->succeeded())->toHaveKey('100000001')
        ...
        ->and(implode('; ', $report->failureReasons()['100000001']))->toContain('did not return a shipment');
});
