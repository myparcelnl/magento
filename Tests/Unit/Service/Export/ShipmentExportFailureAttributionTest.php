...
it('re-sends the chunk without the refused order, so only that order fails', function () {
    ...
    expect($calls)->toHaveCount(2)
        ...
        ->and(array_map('strval', array_keys($report->failureReasons())))->toBe(['3000000033'])
...
it('retries once only, and reports every order that still did not ship', function () {
    ...
    expect($calls)->toHaveCount(2)
        ->and(array_map('strval', array_keys($report->failureReasons())))->toBe(['3000000033', '000000116'])
...
it('does not retry when the response blames nobody', function () {
    ...
    expect($calls)->toHaveCount(1)
        ->and($report->failureReasons())->toBeEmpty()
...
it('does not retry a failure that may have created shipments', function () {
    ...
    // And the admin is told so: "not exported" would invite the retry that bills twice.
    expect($calls)->toHaveCount(1)
        ->and($report->collateral())->toBe(['000000116', '000000117'])
        ->and($report->failureMessages()[0])->toContain('it is unknown whether they shipped')
        ->and($report->failureMessages()[0])->not->toContain('were not exported');
});
