...
function createCollectionFactory(array $existingRows = []): CollectionFactory
{
    ...
    // The service chains addFieldToFilter(...) twice, so each call returns the same collection.
...
it('returns a 64-character lowercase hex token and persists its SHA-256 hash', function () {
    ...
    $service = new TokenService($writer, createCollectionFactory());
...
it('throws AlreadyExistsException and does not persist when hash already exists at another scope', function () {
    ...
    // Any value at any other scope coordinate is sufficient to trigger collision.
    // The service only checks (scope, scope_id) ≠ current, regardless of hash equality —
    // because the collection is already filtered by 'value = $hash' in the query.
    $factory = createCollectionFactory([
        ['scope' => ScopeInterface::SCOPE_WEBSITES, 'scope_id' => 1],
    ]);

    $service = new TokenService($writer, $factory);
