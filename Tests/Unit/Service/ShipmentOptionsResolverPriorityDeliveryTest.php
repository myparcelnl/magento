...
/**
 * The NL and PostNL gates are gone on purpose. Which carrier offers priority delivery where is the
 * API's answer, and per docs/sdk-v11.md the API is the validator: capabilities inform what the UI
 * offers and never gate an export. A chosen option is sent and refused by name, rather than dropped
 * here on a rule that goes stale the moment a carrier gains the option.
 */
it('sends an explicit choice for any country and carrier, leaving the refusal to the api', function () {
    expect(createShipmentOptions('BE', CarrierPostNL::NAME, [
        ShipmentOption::PRIORITY_DELIVERY => true,
    ])->hasPriorityDelivery())->toBeTrue()
        ->and(createShipmentOptions('NL', 'dhlforyou', [
            ShipmentOption::PRIORITY_DELIVERY => true,
        ])->hasPriorityDelivery())->toBeTrue();
});
