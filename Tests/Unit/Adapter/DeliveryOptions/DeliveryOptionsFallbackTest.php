...
it('carries what the fallback shape can carry', function () {
    ...
    expect($options->getCarrier())->toBe('dpd')
        ...
        ->and($options->deliveryTypeValue()->id())->toBe(DeliveryType::EVENING)
...
it('keeps an unstored checkout option null rather than false', function () {
    expect(ShipmentOptions::of([])->hasSignature())->toBeNull()
        ->and(ShipmentOptions::of([])->getInsurance())->toBeNull()
...
it('normalises nested keys in every named constructor, not only through the factory', function () {
    // Whether nested keys are snake_cased used to depend on the caller having gone
    // through DeliveryOptionsFactory. Each named
