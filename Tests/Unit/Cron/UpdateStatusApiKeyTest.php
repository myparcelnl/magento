...
function runPpsCron(
    ...
    // What the order looks like *after* the chain: $trackAfterChain is what its MyParcel track ended
    // up holding, or null for an order the chain could give no track at all.
    ...
    $track->shouldReceive('getOrderId')->andReturn(1);
    ...
    // IteratorAggregate is named explicitly: a mock of the collection alone iterates nothing, and
    // both the order lookup and the track lookup are now one query each rather than one per order.
    $magentoOrders = Mockery::mock(
        Magento\Sales\Model\ResourceModel\Order\Collection::class . ', IteratorAggregate'
    );
    ...
    $magentoOrders->shouldReceive('getIterator')->andReturnUsing(
        static function () use ($shippableOrder) {
            return new ArrayIterator([$shippableOrder]);
        }
    );

    $trackCollection = Mockery::mock(
        Magento\Sales\Model\ResourceModel\Order\Shipment\Track\Collection::class . ', IteratorAggregate'
    );
    foreach (['addFieldToSelect', 'addAttributeToFilter', 'setPageSize', 'setOrder'] as $fluent) {
        $trackCollection->shouldReceive($fluent)->andReturnSelf();
    }
    $trackCollection->shouldReceive('getData')->andReturn([]);
    $trackCollection->shouldReceive('getIterator')->andReturnUsing(
        static function () use ($track, $trackAfterChain) {
            return new ArrayIterator(null === $trackAfterChain ? [] : [$track]);
        }
    );
    ...
    $objectManager->shouldReceive('create')
        ->with(MyParcelNL\Magento\Cron\UpdateStatus::PATH_MODEL_ORDER_TRACK)
        ->andReturn($trackCollection);
