...
5. **Refuse to guess.** An order whose store has no API key configured is reported with a clear, actionable message and excluded from the batch. It must never fall back to another store's key or to an ambient environment value.
...
- [ ] No code path falls back to `getenv('API_KEY')`, `API_KEY_NL` or `API_KEY_BE` when a store's key is empty.
...
The module already resolves the API key per order correctly in most places (`TrackTraceHolder.php:118`, `MagentoOrderCollection.php:203`, `MagentoCollection.php:628`, `SalesOrderStatusHistoryObserver.php:55`). What is missing is the grouping on the send side, plus three call sites that read the key without a store id.
...
Label PDF merging requires `setasign/fpdi`, which beta.31 still declares but no longer uses. It must become an explicit module dependency rather than a transitive one.
...
Two things are easy to get wrong and are worth checking explicitly in review. First, build one API client per key and reuse it across all services for that key, rather than one per key-and-service pair. Second, an empty API key must fail loudly: the SDK's `ShipmentApiFactory::resolveApiKey()` silently falls back to environment variables when handed an empty string, which would send a merchant's parcels to whichever account those variables point at.
