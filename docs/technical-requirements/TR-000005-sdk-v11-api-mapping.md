...
- beta.31 is the target. Later betas are ordinary maintenance, not this requirement. ~~**Confirmed 2026-08-14** — beta.31 is still the highest `v11` tag on the remote.~~ **Superseded 2026-08-28:** beta.32 and beta.33 now exist and the suite is green on beta.33 — `postShipments()` keeps its beta.31 signature, `src/Services` is untouched, and `CapabilitiesOptionsV2` is unchanged. **Evaluated and not taken for this PR.** It fixes nothing this migration needs: `CommonErrorUser` and `CommonResponsesUserError` are byte-identical, so the spec-versus-reality gap in DR-31 survives and the raw body still has to be read. beta.33 also adds `DHL_FREIGHT` to the carrier enum, which the module has no name for — logged and ignored, which is FR-000010 working as designed rather than something to fix.
