...
- The API key is resolved per order from `myparcelnl_magento_general/api/key` at the order's store scope; what is new is the grouping on the send side.
- Scenario 4 is not hypothetical: the SDK factory resolves an empty key from the environment, so the guard runs before the factory is reached. Mechanism in [TR-000007](../technical-requirements/TR-000007-capabilities-retrieval-and-storage.md), defect 2.
- Scenario 6 corrects a defect: the removed `generateReturnConsignments()` used only the first consignment's key.

## Dependencies

- [US-000008](US-000008-admin-prints-one-merged-label-pdf.md) — the same mass action must also produce one merged PDF.
