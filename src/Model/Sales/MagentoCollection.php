...
abstract class MagentoCollection implements MagentoCollectionInterface
{
    ...
    public function getHtmlForGridColumnsByTracks($tracks): array
    {
        ...
        foreach ($tracks as $track) {
            ...
            // A placeholder is not a track number. Letting it through stamps '–' onto
            // sales_order.track_number, which is a display column the PPS cron used to read as
            // process state.
            if ($track['track_number'] && ! in_array($track['track_number'], TrackAndTrace::PLACEHOLDERS, true)) {
        ...
        foreach ($this->getShipmentsCollection() as $shipment) {
            foreach ($this->getTrackByShipment($shipment)->getItems() as $magentoTrack) {
                ...
                foreach (is_array($colli) ? $colli : [] as $collo) {
                    $secondary[] = ['shipment' => $shipment, 'collo' => $collo];
                }
            }
        }

        $this->addSecondaryShipmentTracks($secondary, $claimedIds);
     ...
     *
     * @param array<int,array{shipment: Order\Shipment, collo: object}> $secondary
     * @param array<int,true>                                           $claimedIds ids already on a track
     */
    private function addSecondaryShipmentTracks(array $secondary, array $claimedIds): void
    {
        foreach ($secondary as $entry) {
            ...
            $magentoTrack = $this->setNewMagentoTrack($entry['shipment']);
