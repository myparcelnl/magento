 ...
 * declared properties, so it drops any key the SDK release does not know about. An unknown key has
 * to survive, so the body is decoded as-is and the module's own value objects read it.
 *
...
class Client
{
    ...
    /**
     * @throws \InvalidArgumentException for a value the SDK refuses to send — write strictly
     */
     ...
     * saying so, and a shipment asking about a dropped option gets an answer to a question it did
     * not ask. V2NameMapTest pins which options currently survive the mapping.
     */
