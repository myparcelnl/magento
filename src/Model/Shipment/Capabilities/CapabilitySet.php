...
final class CapabilitySet
{
    ...
    public static function fromApiResults(array $results): self
    {
        ...
        foreach ($results as $index => $result) {
            if (! is_array($result)) {
                // Said out loud, like every other unreadable shape in this layer: silently dropping
                // an entry would make an api that started answering differently look like an account
                // that lost a carrier.
                Logger::notice(sprintf(
                    'MyParcel capabilities: entry %s is a %s, not an object — skipped',
                    (string) $index,
                    gettype($result)
                ));
    ...
    /** Permissive answers yes: offer the option and let the API decide. */
    ...
    /**
     * The option's own properties, from the first matching result that carries it. The insurance range reads
     * min/max through this.
     *
