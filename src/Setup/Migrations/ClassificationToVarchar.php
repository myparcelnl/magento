...
class ClassificationToVarchar
{
     ...
     *
     * The link field is asked for rather than assumed: Commerce with content staging keys the EAV
     * value tables on `row_id`, where a hardcoded `entity_id` fails before a single code moves. On
     * Open Source it resolves to `entity_id` and nothing changes.
     */
    private function moveValues(int $attributeId): void
    {
        ...
        $connection->query(
            "INSERT INTO {$varchar} (attribute_id, store_id, {$linkField}, value)
             SELECT attribute_id, store_id, {$linkField}, CAST(value AS CHAR)
             FROM {$int}
