...
class UpdateStatus
{
    ...
    private function updateStatusPPS(): self
    {
        ...
        $orderIncrementIds = array_keys($fulfilment);
        $orderEntityIds    = [];
    ...
    protected function ordersAwaitingBarcode(): array
    {
        ...
        MyParcelTracks::scopeSelect($withRealBarcode, 't');
        MyParcelTracks::scopeSelect($withAnyTrack, 'a');
        ...
        foreach ($fulfilment as $incrementId => $shipments) {
            $barcodes = array_values(array_unique(array_filter(
                ...
                static function (string $barcode): bool {
                    return TrackAndTrace::isRealBarcode($barcode);
                }
            )));

            if ($barcodes) {
                $barcodesByIncrementId[(string) $incrementId] = $barcodes;
            }
        }

        if (! $barcodesByIncrementId) {
            return 0;
        }

        $orders  = $this->ordersByIncrementId(array_keys($barcodesByIncrementId));
        $onTrack = $this->ordersWithABarcodeOnTrack($orders);
        $written = 0;

        foreach ($orders as $incrementId => $magentoOrder) {
            if (isset($onTrack[(int) $magentoOrder->getId()])) {
                continue;
            }

            $this->setShippedWithoutShipment($magentoOrder, $barcodesByIncrementId[$incrementId]);
            $written++;
        }
    ...
    /**
     * One collection, not a loadByIncrementId() each.
     *
     * @param string[] $incrementIds
     *
     * @return array<string,Order> keyed by increment id
     */
    private function ordersByIncrementId(array $incrementIds): array
    {
        ...
        foreach ($collection as $magentoOrder) {
            $orders[(string) $magentoOrder->getIncrementId()] = $magentoOrder;
        }
    ...
    /**
     * The orders carrying a MyParcel track with something other than a placeholder on it, in one
     * query rather than one track collection per order.
     *
     * @param array<string,Order> $orders
     *
     * @return array<int,true> keyed by order id
     */
    private function ordersWithABarcodeOnTrack(array $orders): array
    {
        $orderIds = [];

        foreach ($orders as $magentoOrder) {
            $orderIds[] = (int) $magentoOrder->getId();
        }
        ...
        $trackCollection = $this->objectManager->create(self::PATH_MODEL_ORDER_TRACK);
        $trackCollection->addAttributeToFilter('order_id', ['in' => $orderIds]);
        MyParcelTracks::scopeCollection($trackCollection);
        ...
        foreach ($trackCollection as $track) {
            ...
            if (TrackAndTrace::isRealBarcode($number)) {
                $found[(int) $track->getOrderId()] = true;
            }
        }
    ...
    private function addOrdersToCollection(array $orderIds): void
    {
         ...
         */
        $now        = new DateTime(sprintf('now -%d day', self::POLL_WINDOW_DAYS));
        $collection = $this->objectManager->create(MagentoCollection::PATH_MODEL_ORDER_COLLECTION);
        $collection
