...
trait NeedsQuoteProps
{
    ...
    /**
     * Never null: with no quote in the session, getQuote() builds an empty one and puts the current
     * store on it, so the caller gets a quote with no items rather than nothing.
     *
     * @throws \Magento\Framework\Exception\LocalizedException when the session cannot produce one
     * @throws NoSuchEntityException
     */
    protected function getQuoteFromCurrentSession(): Quote
    {
        return ObjectManager::getInstance()->get(Session::class)->getQuote();
    }
