...
class ExportResponse
{
     ...
     *
     * The ids travel in `params` for the caller to POST rather than inside the URL. Selecting every
     * order on a large grid put a four-figure id list into one admin URL, which nginx refuses at its
     * 8 KB header limit — after the shipments were created and billed, so the admin saw only a
     * failed download.
     *
    ...
    public function labels(array $shipmentIds, string $requestType, $positions, bool $notify = false): ?array
    {
        ...
        // The chosen A4 positions travel as one comma string and the controller splits them back
        // into an array. Left off entirely when there are none: see LabelPositions.
        ...
        return [
            'url'    => $this->url->getUrl('myparcel/order/PrintMyParcelLabels'),
            'params' => $params,
