...
class ShipmentExportService
{
    ...
    public function fetchLatest(array $shipmentIdsByApiKey): array
    {
        ...
        foreach ($shipmentIdsByApiKey as $apiKey => $shipmentIds) {
            ...
            try {
                $service = (new ShipmentQuery($this->apiProvider->clientFor((string) $apiKey)))
    ...
    public function delete(array $shipmentIdsByApiKey): void
    {
        foreach ($shipmentIdsByApiKey as $apiKey => $shipmentIds) {
            ...
            try {
                $service = (new ShipmentDeleteService((string) $apiKey, $this->apiProvider->clientFor((string) $apiKey)))
                    ->setUserAgents($this->userAgent->map());
    ...
    public function createReturns(array $rowsByApiKey, bool $sendMail): array
    {
        ...
        foreach ($rowsByApiKey as $apiKey => $rows) {
            ...
            try {
                $service = (new ReturnShipmentService((string) $apiKey, $this->apiProvider->clientFor((string) $apiKey)))
                    ->setUserAgents($this->userAgent->map());
    ...
    {
        try {
            $service = (new ShipmentCreateService($apiKey, $this->apiProvider->clientFor($apiKey)))
                ->setUserAgents($this->userAgent->map());
        } catch (Throwable $e) {
